...
const ExifView: React.FC<ExifViewProps> = ({ exif }) => {
    ...
    return (
        <Card className="bg-card text-card-foreground w-full gap-2 border pb-2 shadow-sm">
            <CardHeader className="cursor-pointer pb-3" onClick={toggleExpand}>
                <CardTitle className="flex items-center justify-between text-base leading-none font-semibold tracking-tight">
            ...
            {isExpanded && (
                <CardContent className="grid grid-cols-1 gap-x-6 gap-y-4 pt-0 pb-2 text-sm sm:grid-cols-2">
                    {/* Use a helper component for consistent styling */}
                    {exif.Make && (
                        <ExifField label={t('exif.make')} value={exif.Make} />
                    )}
                    {exif.Model && (
                        <ExifField
                            label={t('exif.model')}
